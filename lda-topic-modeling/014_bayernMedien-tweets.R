...
bayRn <- bayRn[,-1]
bayRn <- cbind(doc_id = 1:106171, bayRn)
# bayRn <- bayRn %>% rename(text = content)
...
# bayRn$text <- chartr(old = "₀₁₂₃₄₅₆₇₈₉", new = "0123456789", bayRn$text)

bayRn$text <- gsub("\\n", " ", bayRn$text, perl = TRUE)
bayRn$text <- gsub("lt3", " ", bayRn$text, perl = TRUE)

bayRn$text <- gsub("…", " ", bayRn$text, perl = TRUE)
# bayRn$text <- gsub("–", "", bayRn$text, perl = TRUE) ## in python mit re.sub gemacht für extra word hyphens und "–"
...
# bayRn$text <- gsub("\\p{Cn}|\\p{Ps}|\\p{Pe}|\\p{Pi}|\\p{Pf}|\\p{Po}|\\p{S}", "", bayRn$text, perl = TRUE)
bayRn$text[2]
bayRn$text[43]
bayRn$text[83918]
bayRn$text[100948]
# sample_überprüfung <- as_tibble(bayRn[sample(nrow(bayRn), 150), ])
# # chartr(old = "₀₁₂₃₄₅₆₇₈₉", new = "0123456789", sample_überprüfung$text)
# sample_überprüfung

# ## versuche des emoji replacements
# textclean::replace_emoji(bayRn$text, emoji_dt = lexicon::hash_emojis)
# utf8::utf8_encode(textclean::replace_emoji_identifier(bayRn$text[1:10], emoji_dt = lexicon::hash_emojis_identifier))
# 
# 
...
# 
# replace_non_ascii(replace_emoji(bayRn$text))
# 
...
corpusBay <- Corpus(DataframeSource(bayRn))
...
# processedCorpus_bay <- tm_map(corpusBay, removeWords, stopwords::stopwords(language = "de", source = "marimo"))
# processedCorpus_bay <- tm_map(processedCorpus_bay, removeWords, stopwords::stopwords(language = "de", source = "stopwords-iso"))
# processedCorpus_bay <- tm_map(processedCorpus_bay, removeWords, stopwords::stopwords(language = "en", source = "stopwords-iso"))
# processedCorpus_bay <- tm_map(processedCorpus_bay, removeWords, myStopwordsDE)
processedCorpus_bay <- tm_map(corpusBay, removeWords, allstopwords)
...
processedCorpus_bay <- tm_map(processedCorpus_bay, removePunctuation, preserve_intra_word_dashes = TRUE) ## in python erledigt
# processedCorpus <- tm_map(processedCorpus, removeNumbers)
# processedCorpus <- tm_map(processedCorpus, stemDocument, language = "de")
processedCorpus_bay <- tm_map(processedCorpus_bay, stripWhitespace) ## unsicher ob gebraucht, da keine absätze drin -- lass mal laufen lieber
# processedCorpus <- rm_non_words(processedCorpus)

# pal <- RColorBrewer::brewer.pal(8, "Dark2")
# x11()
# wordcloud(processedCorpus_bay, min.freq=10, max.words = 150, random.order = TRUE, col = pal) # schaut sinnvoll aus von wörtern her

# View(processedCorpus)
# summary(processedCorpus)

minimumFrequency <- 5
DTMbay <- DocumentTermMatrix(processedCorpus_bay, control = list(bounds = list(global = c(minimumFrequency, Inf))))
# have a look at the number of documents and terms in the matrix
dim(DTMbay)
DTMbay

## haut die observationen raus, die 0 als row sum haben ( also kein inhalt )
sel_idx <- slam::row_sums(DTMbay) > 0
DTMbay <- DTMbay[sel_idx, ]
bayRn <- bayRn[sel_idx, ]
...
# save(resultBay_all, file = "CaoJuan-Deveaud_results/BAYERN_alleKoeff_k2-150_10er.RData")
...
set.seed(3)
topicModel_kMin <- LDA(DTMbay, kMin, method="Gibbs", control=list(iter = 500, verbose = 10, best=T, alpha=0.01)) ## iter default = 2000, 500 für schneller
...
set.seed(4)
topicModel_kMedium <- LDA(DTMbay, kMedium, method="Gibbs", control=list(iter = 500, verbose = 10, best=T, alpha=0.01)) ## iter default = 2000, 500 für schneller
...
bayRn_zuordnungTest <- full_join(x=bayRn, y=tmResultMedium_df, by="doc_id")

################

set.seed(5)
topicModel_kMax <- LDA(DTMbay, kMax, method="Gibbs", control=list(iter = 500, verbose = 10, best=T, alpha=0.01)) ## iter default = 2000, 500 für schneller
